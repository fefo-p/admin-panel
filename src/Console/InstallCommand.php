<?php
    ...
    class InstallCommand extends Command
    {
        ...
        public function handle()
        {
            ...
            $this->installPolicyAfter('protected $policies = [',
                                      'App\Models\User::class => FefoP\AdminPanel\Policies\UserPolicy::class');
            $this->installPolicyAfter('protected $policies = [',
                                      'FefoP\AdminPanel\Models\Role::class => FefoP\AdminPanel\Policies\RolePolicy::class');
            $this->installPolicyAfter('protected $policies = [',
                                      'FefoP\AdminPanel\Models\Permission::class => FefoP\AdminPanel\Policies\PermissionPolicy::class');

            // Configure Email Verification...
            if ( $this->option('verification') ) {
                $this->info('Setting use of email verification');
                $this->replaceInFile('// Features::emailVerification(),', 'Features::emailVerification(),',
                                     config_path('fortify.php'));
                $this->replaceInFile('class User extends Authenticatable',
            ...
            if ( $this->option('profile-image') ) {
                ...
                $this->replaceInFile('        // Features::profilePhotos(),', '        Features::profilePhotos(),',
                                     config_path('jetstream.php'));
                ...
                $this->replaceInFile($last_line, $this->override_profile_photo_url().PHP_EOL.' } ',
                                     app_path('Models/User.php'));
            }
            ...
            $this->addLineAfter('use Laravel\Fortify\TwoFactorAuthenticatable', 'use Spatie\Permission\Traits\HasRoles',
                                app_path('Models/User.php'));
            ...
            $this->addLineAfter('use Spatie\Permission\Traits\HasRoles', 'use Illuminate\Database\Eloquent\SoftDeletes',
                                app_path('Models/User.php'));

            // Publish needed migrations...
            $this->info('Publishing Spatie permission migrations.');
            $this->callSilent('vendor:publish',
                              [ '--provider' => 'Spatie\Permission\PermissionServiceProvider', '--force' => false ]);
        ...
        {
            if ( !Str::contains($authSPFile = file_get_contents(app_path('Providers/AuthServiceProvider.php')),
                                $name) ) {
...
EOF;
        ...
        protected function create_roles_and_permissions($user)
        {
            ...
            // Asignar todos los permisos al Administrador general (que no el AG)
            // menos los de administrar usuarios y permisos (sólo asignar el ver)
            $role = Role::first();
